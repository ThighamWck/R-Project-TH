"""
#################### 
# Fourier Neural Operator trainer for Curved domains - for Fourth Year Project
# Author - Thomas Higham
# Date - 18/06/2025
# University of Warwick
#####################
I use "config_curbed_FNO.json" to specify my hyperparameters
# """
...
# LOAD CONFIGURATION
# ===============================
def load_config(config_path="config_curved_FNO.json"):
...
class PDEOperatorDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Extract input features (x, y, b1, b2, finv_val) and target (rho_val)
        x = torch.tensor(group_df[['x_ij', 'y_ij', 'b1', 'b2', 'finv_val']].values, dtype=torch.float32)

        # Extract 'rho_val' as the target variable
...
class NPZOperatorDataset(Dataset):
    def __init__(self, data, grid_size):
        ...
        self.b1 = data['b1']
        self.b2 = data['b2']
        self.finv_val = data['finv_val']
        self.rho_val = data['rho_val']
        self.N = self.x_ij.shape[0]
    ...
    def __getitem__(self, idx):
        ...
        x = np.stack([
            ...
            self.b2[idx],
            self.finv_val[idx]
        ], axis=0)  # [5, grid_size, grid_size]
        target = self.rho_val[idx]  # [grid_size, grid_size]
...
def train_model(model, dataloader, config, optimizer):
    ...
    checkpoint = {
        ...
        'config': config  # Optionally save config for reproducibility
    }
...
def main():
    ...
    train_model(model, dataloader, config, optimizer)
